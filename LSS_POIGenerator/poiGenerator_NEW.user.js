...
// @author       Lennard[TFD]
// @match        https://www.leitstellenspiel.de/mission_positions/new
// @match        https://www.missionchief.com/mission_positions/new
// @match        https://www.meldkamerspel.com/mission_positions/new
...
(function() {
    ...
    //Get JS addMarker and parse to JSON
    let jsString = document.getElementsByTagName("script")[0].innerHTML;
    var replaced = "[" + jsString.slice(jsString.indexOf("missionPositionMarkerAdd"), jsString.indexOf("if (typeof(prepared_position)")).trim().replace(/missionPositionMarkerAdd\(/g, "").replace(/\\n/g, "").replace(/\);/g, ",") + "]";
    replaced = JSON.parse(replaced.replaceAt(replaced.lastIndexOf(","), " "));

    //Generate List of locations for POI {"POIName": ["lat,lng","lat,lng"]}
    var mappedPois = {};
    for(var[key,value] of Object.entries(replaced))
    {
        if(!mappedPois[value.caption]) mappedPois[value.caption] = [];
        mappedPois[value.caption].push(value.latitude + "," + value.longitude);
    }
    ...
    async function createPOIs(poiId, poi) {
        ...
        let addressRequest = await locationToAddress(lat, lng);
